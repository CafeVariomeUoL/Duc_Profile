$(function(){
  ...
  $(".next").click(function () {
    ...
    if(btnId == "selectionToCon"){
      let counter =0;
      $(document).find('.tsel').each((i,v)=>{
        if($(v).prop('checked') == true){counter += 1;}
      })
      if(counter == 0){
       
        Swal.fire({
          icon: 'error',
          text: 'Please select your terms group',
      })
      return;
  ...
  { 
    ...
    // }
    if(id == 'sinfo'){
        $("#progressbar li").eq($("fieldset").index(kids[6])).addClass('active');
      if(index!=6){
